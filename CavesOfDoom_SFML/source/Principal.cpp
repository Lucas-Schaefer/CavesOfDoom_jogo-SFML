...
Principal::Principal() {
    if (pGrafico == nullptr) {
        std::cout << "ERROR::Principal nao foi possivel criar o GerenciadorGrafico" << std::endl;
        exit(1);
    }
    if (pEvento == nullptr) {
        std::cout << "ERROR::Principal nao foi possivel criar um GerenciadorEvento" << std::endl;
        exit(1);
    }
